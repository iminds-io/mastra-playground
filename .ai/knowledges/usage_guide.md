...
This guide is the coworker handoff manual for running, deploying, operating, testing, and extending `hono-workspace`.

`hono-workspace` is a Hono + Mastra AI workspace app. It provides a Firebase-authenticated project API, project-scoped chat channels/threads, workspace-backed Mastra agents, document summarization, supervisor-agent analysis, Mastra-native agent/workflow endpoints, and runtime agent overrides through `@mastra/editor`.
...
| Package | Purpose |
|---|---|
| `packages/platform` | Shared business logic, DB repositories, Mastra agents/workflows/tools, workspace services. |
...
- **Tier B domain API:** `/api/projects/:projectId/...` routes that enforce Firebase auth, project authorization, workspace resolution, and product-specific request/response shapes.
- **Tier A Mastra API:** `/api/mastra/*` routes exposed by `@mastra/hono` for registered agents, workflows, and editor-backed stored overrides.

Use Tier B for product behavior. Use Tier A for generic Mastra execution, inspection, editor operations, tests, and operator workflows.
...
- **Cloudflare R2** bucket and S3-compatible access keys for Worker workspace storage.
...
```bash
pnpm --filter @hono-workspace/worker dev
```
...
- The Worker creates `mastra`, the Neon HTTP DB adapter, and the R2-backed `workspaceFactory` per request.
...
pnpm exec wrangler secret put WORKSPACE_ROOT
...
```bash
pnpm --filter @hono-workspace/web build
```
...
curl -X POST "$API_BASE_URL/api/projects/$PROJECT_ID/channels/$CHANNEL_ID/threads/$THREAD_ID/messages" \
  -H "Authorization: Bearer $FIREBASE_ID_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"message":"Summarize the workspace state."}'
```
...
This route authorizes project access, resolves the workspace, builds a Mastra `RequestContext`, and calls `projectAgent.generate()`.

---

## Summarization

Summarize workspace paths through the Tier B route:
...
curl -X POST "$API_BASE_URL/api/projects/$PROJECT_ID/supervise" \
  -H "Authorization: Bearer $FIREBASE_ID_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"prompt":"Review the workspace for implementation risks.","paths":["README.md","packages/platform/src"]}'
```
...
curl -X POST "$API_BASE_URL/api/projects/$PROJECT_ID/supervise?status=draft" \
  -H "Authorization: Bearer $FIREBASE_ID_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"prompt":"Review this workspace."}'
```

---

## Mastra-Native Tier A Routes
...
Tier A routes do not automatically provide the project `Workspace`. Prefer Tier B routes for product flows that need real project workspace access.
...
1. Create a specialist with `buildWorkspaceAgent()`.
2. Pick `workspaceReadOnlyToolkit` or `workspaceToolkit`.
...
6. Add a Tier B service/route only if project authorization and workspace resolution are required.
...
Current verified counts as of 2026-04-23:
...
Every workspace-aware agent uses:
...
| `Cannot perform I/O on behalf of a different request` | Worker request-scoped I/O object leaked across requests. | Ensure pools/workspaces/Mastra are created per request. |
